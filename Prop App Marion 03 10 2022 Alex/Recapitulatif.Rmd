...
Etape 1 : App / Test 
```{r}
index <- sample(nrow(stars.V2), nrow(stars.V2)*0.75)
data.train <- stars.V2[index, ] #Echantillon d’apprentissage
data.test <- stars.V2[-index, ] #Echantillon de test
```

```{r}
res <- rpart(type~., data=data.train, method='class')
plotcp(res)
visTree(res, main = "Classification des étoiles", legend=FALSE)
```
Etape 2 : prédiction après élagage optimal
```{r}
tree_opt <- prune(res,cp=res$cptable[which.min(res$cptable[,4]),1])

pred<-predict(tree_opt,newdata=data.test, type="class")
```

Etape 3 : Matrice de confusion 
```{r}
mc <- confusionMatrix(data.test$type,pred)
mc$table
mc$overall[[1]]
```
